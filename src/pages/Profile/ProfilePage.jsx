...
const ProfilePage = () => {
  ...
  // ---------- Skills & Interests modals (kept simple) ----------
  ...
  const handleBecomeAlumnus = async () => {
    try {
      setPromoting(true);
      await becomeAlumnus();
    } finally {
      ...
      <div className="space-y-6">
        ...
        >
          <form onSubmit={handleSaveCover} className="space-y-5">
            ...
            {coverError && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 p-3 rounded-lg bg-error/10 border border-error/20 text-error text-xs font-semibold">
                <FiAlertCircle size={15} className="shrink-0" />
            ...
            {coverSuccess && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 p-3 rounded-lg bg-[#dcfce7] border border-[#bbf7d0] text-[#166534] text-xs font-semibold">
                <FiCheck size={15} className="shrink-0" />
            ...
            <div className="flex flex-col gap-2">
              <Button onClick={handleRemoveCover} variant="outline" size="sm" className="w-full gap-2 border border-outline text-error hover:bg-error/10 hover:text-error hover:border-error/20 transition-all duration-200">
                <FiTrash2 size={14} />
            ...
            <div className="flex gap-3 justify-end pt-3.5 border-t border-outline-variant">
              <Button onClick={() => setIsCoverModalOpen(false)} variant="outline" size="sm" disabled={isCoverUploading}>Cancel</Button>
              <Button type="submit" variant="primary" size="sm" disabled={isCoverUploading} className="min-w-[80px]">{isCoverUploading ? 'Saving...' : 'Save Banner'}</Button>
            </div>
          </form>
        </Modal>
