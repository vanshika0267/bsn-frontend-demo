import type { UserRole, RoleDetectionResult } from '../types';
...
export const ROLE_META: Record<Exclude<UserRole, null>, {
  label: string;
  color: string;
  bg: string;
  description: string;
...
export function getRoleLoginFields(role: UserRole) {
  switch (role) {
    case 'student':
      return {
        title: 'Student Login',
    ...
    case 'faculty':
      return {
        ...
        fields: ['department', 'institution', 'designation'] as const,
    ...
    case 'recruiter':
      return {
        ...
        fields: ['company', 'position', 'industry'] as const,
