...
export default function GitHubConnect() {
  ...
  const handleConnect = async (rawInput?: string) => {
  ...
  return (
    <div className="bg-white rounded-[24px] border border-[--color-border-gray] shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-[--color-border-gray] flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display text-[18px] font-semibold text-[--color-secondary-navy]">GitHub</h3>
        ...
        {username && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="text-[12px] px-3.5 py-2 rounded-xl border border-[--color-border-gray] hover:bg-slate-50 disabled:opacity-50"
            >
      ...
      <div className="p-6 space-y-6">
        {!username ? (
          <div className="bg-[#F8FAFC] border border-[--color-border-gray] rounded-[18px] p-5">
            ...
            <div className="flex gap-3 flex-col sm:flex-row">
              <input
                ...
                className="flex-1 px-4 py-[12px] rounded-xl border border-[--color-border-gray] focus:outline-none focus:ring-[3px] focus:ring-slate-900/8 focus:border-slate-800 text-[14px] bg-white"
              />
              <button
                onClick={() => handleConnect()}
                disabled={loading || !input.trim()}
                className="px-5 py-[12px] rounded-xl bg-slate-900 text-white text-[14px] font-[500] hover:bg-black transition disabled:opacity-50 whitespace-nowrap"
              >
        ...
        {error && (
          <div className="text-[13px] px-4 py-3 rounded-2xl bg-red-50 text-red-700 border border-red-200">
            {error}
          </div>
        )}
